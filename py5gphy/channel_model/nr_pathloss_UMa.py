...
class NrPathlossUMa():
    """ UMa	Urban Macro refer to Table 7.4.1-1: Pathloss models
    """
    def __init__(self, hUT=1.5, d2D=20, hE=1, optional=False):
        self.config = {}
        self.config["hBS"] = 25 #BS antenna height in meter
    ...
    def cal(self, freq_in_Hz, LOS):
        ...
        if d2D <= 18:
            ...
            C_hUT = 0 if hUT<= 13 else ((hUT-13)/10)**1.5
            Pr_LOS = (18/d2D + np.exp(-d2D/63)*(1 - 18/d2D)) * (1 + C_hUT * 5/4 * (d2D/100)**3 * np.exp(-d2D/150))
        ...
        if d2D <= dBP:
            #PL1
            PL_UMa_LOS = 28.0 + 22*np.log10(d3D) + 20*np.log10(fc)
        else:
            #PL2
            PL_UMa_LOS = 28.0 + 40*np.log10(d3D) + 20*np.log10(fc) \
                        - 9*np.log10(dBP**2 + (hBS-hUT)**2)
                    
        if LOS == False:
            #NLOS
            if optional == False:
                PL_UMa_NLOS = 13.54 + 39.08*np.log10(d3D) + 20*np.log10(fc) - 0.6*(hUT-1.5)
                PL_UMa_NLOS = max(PL_UMa_LOS, PL_UMa_NLOS)
                SF_std = 6
            else:
                PL_UMa_NLOS = 32.4 + 20*np.log10(fc) + 30*np.log10(d3D)
                SF_std = 7.8
            return [PL_UMa_NLOS, SF_std, Pr_LOS]
        else:
            #LOS
            return [PL_UMa_LOS, SF_std, Pr_LOS]

if __name__ == "__main__":
    nrPathlossUMa = NrPathlossUMa()

    print(nrPathlossUMa.get_config())
    default_hUT = nrPathlossUMa.config["hUT"]
    default_d2D = nrPathlossUMa.config["d2D"]

    print(nrPathlossUMa.get_hE_distribution())
    nrPathlossUMa.config["hUT"] = 20
    nrPathlossUMa.config["d2D"] = 18
    print(nrPathlossUMa.get_hE_distribution())
    nrPathlossUMa.config["d2D"] = 3000
    print(nrPathlossUMa.get_hE_distribution())

    nrPathlossUMa.config["hUT"] = default_hUT
    nrPathlossUMa.config["d2D"] = default_d2D
    freq_in_Hz = 2e9
    LOS = True
    [PL_no_shadow, SF_std, Pr_LOS] = nrPathlossUMa.cal(freq_in_Hz, LOS)
    ...
    nrPathlossUMa.config["optional"] = True
    [PL_no_shadow, SF_std, Pr_LOS] = nrPathlossUMa.cal(freq_in_Hz, LOS)
