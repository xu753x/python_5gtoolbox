...
def data_control_separate(LLr, pusch_config,
        DMRS_symlist,RMinfo, Qm):
    """separate 38.212 6.2.7 Data and control

    output:
    g_ulsch, g_ack, g_csi1, g_csi2
    """
    ...
    #step 6: from LLr to gbarLkv_seq matrix
    gbarLkv_seq = np.zeros((NrOfSymbols,RBSize*12, NL*Qm))
    t = 0
    for L in range(NrOfSymbols):
        for j in range(MULSCH[L]):
        ...
        while mACKcount1 < GACK:
            if MbarUCI[L] > 0:
                ...
                for j in range(mREcount):
                    k = phibarUCI[L][j*d]
                    for v in range(NL*Qm):
                        g_ack[mACKcountall] = gbarLkv_seq[L][k][v]
                        mACKcountall += 1
                        mACKcount1 += 1
    ...
    GCSI1_1 = RMinfo['Euci_CSI1']
        
    g_csi1 = np.array([])
    if EnableCSI1*NumCSI1Bits >0:
        g_csi1 = np.zeros(GCSI1_1)
        mCSI1count = 0
        mCSI1countall = 0
        L=LCSI1
        while MbarUCI[L] - Mbarphibarrvd[L] <= 0:
        ...
        while mCSI1count < GCSI1_1:
            if MbarUCI[L] - Mbarphibarrvd[L] > 0:
                ...
                for j in range(mREcount):
                    k = phibartemp[j*d]
                    for v in range(NL*Qm):
                        g_csi1[mCSI1countall] =gbarLkv_seq[L][k][v]
                        mCSI1countall += 1
                        mCSI1count += 1
    ...
    #CSI 2
    GCSI2_1 = RMinfo['Euci_CSI2']
    g_csi2 = []
    if EnableCSI2*NumCSI2Bits >0:
        g_csi2 = np.zeros(GCSI2_1)
        mCSI2count = 0
        mCSI2countall = 0
        L=LCSI1
        while MbarUCI[L] <= 0:
        ...
        while mCSI2count < GCSI2_1:
            if MbarUCI[L] > 0:
                ...
                for j in range(mREcount):
                    k = phibarUCI[L][j*d]
                    for v in range(NL*Qm):
                        g_csi2[mCSI2countall] =gbarLkv_seq[L][k][v]
                        mCSI2countall += 1
                        mCSI2count += 1
        ...
        for L in range(NrOfSymbols):
            if MbarULSCH[L] > 0:
                for j in range(MbarULSCH[L]):
                    k = phibarULSCH[L][j]
                    for v in range(NL*Qm):
                        g_ulsch.append(gbarLkv_seq[L][k][v])
                        mULSCHcount += 1
    
    g_ulsch = np.array(g_ulsch)
    ...
    return g_ulsch, g_ack, g_csi1, g_csi2