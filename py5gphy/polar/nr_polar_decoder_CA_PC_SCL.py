...
if __name__ == "__main__":
    from tests.polar import test_nr_polar_decoder_CA_PC_SCL

    testlists = test_nr_polar_decoder_CA_PC_SCL.get_testvectors()
    for _ in range(20):
        for idx, testcase in enumerate(testlists):
            print("count= {}".format(idx))
            test_nr_polar_decoder_CA_PC_SCL.test_nr_polar_SCL_decoder(testcase)

    for _ in range(20):
        for idx, testcase in enumerate(testlists):
            print("count= {}".format(idx))
            test_nr_polar_decoder_CA_PC_SCL.test_nr_polar_SCL_decoder_no_noise(testcase)
