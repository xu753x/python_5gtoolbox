...
def nr_decode_polar_SC(LLRin, E, K, nMax, iIL):
    """ SC polar decoder
    input:
...
def PolarSCDecoder(LLRin, F, N):
    ...
    #SC main loop
    for n in range(N):
        #print("traditional SC in main func, phase= {}".format(n))
...
def recursivelyUpdateB(path,layer,phase):
    ...
    for branch in range(2**(m-layer)):
        ...
        #print("recursivelyUpdateB B_value set [{}, {}, {}]".format(layer-1, 2*branch+1, newphase))
...
def recursivelyCalcLLR(path,layer, phase):
    ...
    if layer == 0:
        return
    newphase = phase // 2
    #print("cal LLR_value [{}, x, {}]".format(layer, phase))
    m = path.m
    if (phase % 2) == 0:
...
if __name__ == "__main__":
    from tests.polar import test_nr_polar_decoder_SC
    import time
    ...
    testlists = test_nr_polar_decoder_SC.get_testvectors()
    for _ in range(2):
        for idx, testcase in enumerate(testlists):
            print("count= {}".format(idx))
            test_nr_polar_decoder_SC.test_nr_polar_SC_decoder_no_noise(testcase)

    for _ in range(2):
        for idx, testcase in enumerate(testlists):
            print("count= {}".format(idx))
            test_nr_polar_decoder_SC.test_nr_polar_SC_decoder(testcase)
