...
from tests.nr_pucch import nr_pucch_format2_testvectors
...
def get_testvectors():
    ...
    file_lists = []
    for f in os.listdir(path):
        if f.endswith(".mat") and f.startswith("nr_pucchformat2_testvec"):
            file_lists.append(path + '/' + f)
                        
    return file_lists

@pytest.mark.parametrize('filename', get_testvectors())
def test_nr_pucchformat2(filename):
    #read data
    matfile = io.loadmat(filename)
    codeword_out, symbols_out, fd_slot_data_ref, pucchformat2_tvcfg = \
        nr_pucch_format2_testvectors.read_pucch_format2_testvec_matfile(matfile)
    carrier_config, pucch_format2_config = \
        nr_pucch_format2_testvectors.gen_pucchformat2_testvec_config(pucchformat2_tvcfg)
    ...
    for slot in range(slot_num):
        fd_slot_data, RE_usage_inslot = \
            nr_slot.init_fd_slot(carrier_config['num_of_ant'], nrPUCCHFormat2.carrier_prb_size)
    
        sfn = 1 #pucch period is 1 slots, so any value is OK
    
        fd_slot_data, RE_usage_inslot = \
            nrPUCCHFormat2.process(fd_slot_data, RE_usage_inslot,sfn,slot)
