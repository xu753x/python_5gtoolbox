...
def get_channel_parameter_list():
    ...
    #model_format, Timeoff_ns, rho, fm_inHz, fDo_in_Hz,DSdesired,[alpha,beta] for MIMO spat
        ...
        ["TDL-B",     1,           0,   0,      0,       100,       [0,0] ],
        ["TDL-C",     2,           0,   0,      0,       100,       [0,0] ],
        ["TDL-D",     3,           0,   400,      0,       100,       [0,0] ],
        ["TDL-E",     4,           0,   400,      0,       100,       [0,0] ],
        ["TDL-A",     100,           0,   0,      0,       100,       [0,0] ],
        ["TDL-B",     5,           0,   0,      0,       200,       [0.3,0.9] ],
        ["TDL-C",     6,           0,   0,      0,       400,       [0.9,0.9] ],
        ["TDL-B",     250,           0,   400,      0,       10,       [0,0] ],
        ["TDL-C",     24,           0,   400,      0,       200,       [0.3,0.3] ],
...
def test_nr_pdsch_rx_TDL_basic(pdsch_carrier_testvectors, CE_config,channel_parameter,CEQ_algo):
    "test PDSCH Rx processing TDL channel, low correlation MIMO, "
    ...
    if status:
        print(f"pass mcsidx{mcs_index},Nt{Nt},Nr{Nr},{CEQ_algo},model_format{model_format},Timeoff_ns{Timeoff_ns},fDo_in_Hz{fDo_in_Hz},alpha{alpha},beta{beta},Elapsed Time: {elapsed_time:.2f} seconds,TO_est{TO_est*10**9:.2f}ns,H_est_power={H_est_power}dB, cov_m_power={cov_m_power}dB ")
    else:
        print(f"failed mcsidx{mcs_index},Nt{Nt},Nr{Nr},{CEQ_algo},model_format {model_format},Timeoff_ns{Timeoff_ns},fDo_in_Hz{fDo_in_Hz},alpha{alpha},beta{beta},Elapsed Time: {elapsed_time:.2f} seconds,TO_est{TO_est*10**9:.2f}ns,H_est_power={H_est_power}dB, cov_m_power={cov_m_power}dB ")

@pytest.mark.parametrize('pdsch_carrier_testvectors', get_pdsch_carrier_testvectors_list())
@pytest.mark.parametrize('CE_config', get_CE_config_list())
@pytest.mark.parametrize('channel_parameter', get_channel_parameter_list())
@pytest.mark.parametrize('CEQ_algo', get_channel_equ_config_list())
def test_nr_pdsch_rx_TDL_correlated_MIMO(pdsch_carrier_testvectors, CE_config,channel_parameter,CEQ_algo):
    "test PDSCH Rx processing on TDL channel with middle and high correlation, "
    ...
    cov_m_power = 10*np.log10(np.abs(nrChannelEstimation.cov_m[0,0,:,:].reshape(Nr*Nr)))
    if status:
        print(f"pass en TO comp,mcsidx{mcs_index},Nt{Nt},Nr{Nr},{CEQ_algo},model_format{model_format},Timeoff_ns{Timeoff_ns},fDo_in_Hz{fDo_in_Hz},alpha{alpha},beta{beta},Elapsed Time: {elapsed_time:.2f} seconds,TO_est{TO_est*10**9:.2f}ns,H_est_power={H_est_power}dB, cov_m_power={cov_m_power}dB ")
    else:
        print(f"failed en TO comp,mcsidx{mcs_index},Nt{Nt},Nr{Nr},{CEQ_algo},model_format {model_format},Timeoff_ns{Timeoff_ns},fDo_in_Hz{fDo_in_Hz},alpha{alpha},beta{beta},Elapsed Time: {elapsed_time:.2f} seconds,TO_est{TO_est*10**9:.2f}ns,H_est_power={H_est_power}dB, cov_m_power={cov_m_power}dB ")
